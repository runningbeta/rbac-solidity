...
const RBACMock = artifacts.require('mocks/RBACMock');
...
const ROLE_ADVISOR = Web3Utils.soliditySha3('advisor');
...
contract('RBAC', function (accounts) {
  ...
  context('in normal conditions', () => {
    ...
    it('allows an admin to remove an advisor\'s role who then can\'t call #onlyAdvisorsCanDoThis', async () => {
      await mock.removeAdvisor(advisors[0], { from: admin })
        .should.be.fulfilled;
        await expectThrow(
          mock.checkRole(advisors[0], ROLE_ADVISOR)
        );
      await expectThrow(
        mock.onlyAdvisorsCanDoThis({ from: advisors[0] })
      );
    });
    it('allows admins to #adminRemoveRole who then can\'t call #onlyAdvisorsCanDoThis', async () => {
      await mock.adminRemoveRole(advisors[3], ROLE_ADVISOR, { from: admin })
        .should.be.fulfilled;
      await expectThrow(
        mock.checkRole(advisors[3], ROLE_ADVISOR)
      );
      await expectThrow(
        mock.onlyAdvisorsCanDoThis({ from: advisors[3] })
      );
    });
    ...
    it('announces a RoleAdded event on addRole', async () => {
      ...
      await mock.checkRole(futureAdvisor, ROLE_ADVISOR)
    });
    ...
    it('announces a RoleRemoved event on removeRole', async () => {
      ...
      await expectThrow(
        mock.checkRole(futureAdvisor, ROLE_ADVISOR)
      );
    });
  });
